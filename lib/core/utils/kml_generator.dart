...
<kml xmlns="http://www.opengis.net/kml/2.2"
  ...
  /// Increased font sizes significantly for 4K screen readability.
  ...
  static String _riskToKmlColor(RiskLevel level) {
    switch (level) {
      case RiskLevel.low:
        return 'ff00aa00'; // green (AABBGGRR in KML)
      case RiskLevel.medium:
        return 'ff0078ff'; // orange
      case RiskLevel.high:
        return 'ff0000ff'; // red (AABBGGRR)
    }
  }
  ...
  /// Master screen shows only this pin — no text, no data cards.
  static String plantPinKml({
    required PowerPlant plant,
    required RiskLevel riskLevel,
  }) {
    // Use pre-colored Google paddle icons — guaranteed to render correctly on LG.
    // No color tinting needed: the icon itself carries the right color.
  ...
  ///
  /// The [lon] parameter should already be offset for the rightmost slave screen
  /// position (same formula as the region overlay).
  /// Climate snapshot section is omitted entirely if [climateData] is null
  /// or if temperature is not available.
  static String plantDetailBalloon({
    ...
    // ── Plant type icon (HTML entity / text) ──────────
    final typeIcon = _plantTypeIcon(plant.primaryFuel.csvLabel);

    // ── Top 3 risk dimensions (scaled 0–10, thresholds per spec) ──
    final dims = [
    ...
    final dimRows = dims
        .map((d) {
          final name = d.$1;
          final raw = d.$2; // 0–100 scale
          final scaled = raw / 10.0; // 0–10 scale for display
    ...
    // ── Climate snapshot (omit entirely if data absent) ──
...
$weatherSection
  ...
  static String _plantTypeIcon(String csvLabel) {
    ...
    if (lower.contains('wind')) return '&#127788;'; // 🌬 (approximated)
